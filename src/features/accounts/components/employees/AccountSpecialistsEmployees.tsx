  ...
  },
  {
    ...
    render: (row) => {
      ...
      const toggleLead = useMutation({
        ...
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: ["accounts", "employees", "account-specialists"] });
        },
      });
  ...
  },
  {
    ...
    render: (row) =>
      row.status.state === "ACTIVE" ? (
        ...
        <Text c="gray">OFFLINE ({row.status.lastSeen.toLocaleString()})
        </Text>
      ),
  },
  {
    ...
    render: (row) => {
      ...
      const deactivate = useMutation({
        ...
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ["accounts", "employees", "account-specialists"] }),
      });
      ...
      return (
        <Menu shadow="md" width={180}>
          ...
          <Menu.Dropdown>
            <Menu.Item onClick={() => navigate(`/accounts/employees/account-specialists/${row.id}`)}>
              See Profile
            </Menu.Item>
...
export function AccountSpecialistsEmployees() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["accounts", "employees", "account-specialists", searchQuery, perPage],
  ...
  return (
    ...
    >
      <AppTable
        ...
        onRowClick={(row) => navigate(`/accounts/${tab}/account-specialists/${row.id}`)}
      />
    </PageCard>
  );
}
