      ...
      const toggleLead = useMutation({
        ...
        onSuccess: () => {
          queryClient.invalidateQueries({
            queryKey: ["accounts", "employees", "marketing"],
          });
        },
      });
    ...
    render: (row) => {
      ...
      return (
        <Menu shadow="md" width={180}>
          ...
          <Menu.Dropdown>
            <Menu.Item
              onClick={() => navigate(`/accounts/employees/marketing/${row.id}`)}
            >
              See Profile
            </Menu.Item>
...
export function MarketingEmployees() {
  const navigate = useNavigate();
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["accounts", "employees", "marketing", searchQuery, perPage],
  ...
  return (
    ...
    >
      <AppTable
        ...
        onRowClick={(row) => navigate(`/accounts/${tab}/marketing/${row.id}`)}
      />
    </PageCard>
  );
}
