      ...
      const toggleLead = useMutation({
        ...
        onSuccess: () => {
          queryClient.invalidateQueries({
            queryKey: ["accounts", "employees", "operations"],
          });
        },
      });
    ...
    render: (row) => {
      ...
      return (
        <Menu shadow="md" width={180}>
          ...
          <Menu.Dropdown>
            <Menu.Item
              onClick={() => navigate(`/accounts/employees/operations/${row.id}`)}
            >
              See Profile
            </Menu.Item>
...
export function OperationsEmployees() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["accounts", "employees", "operations", searchQuery, perPage],
  ...
  return (
    ...
    >
      <AppTable
        ...
        onRowClick={(row) => navigate(`/accounts/${tab}/operations/${row.id}`)}
      />
    </PageCard>
  );
}
