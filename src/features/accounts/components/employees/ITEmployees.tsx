      ...
      const toggleLead = useMutation({
        ...
        onSuccess: () => {
          queryClient.invalidateQueries({
            queryKey: ["accounts", "employees", "it"],
          });
        },
      });
    ...
    render: (row) => {
      ...
      return (
        <Menu shadow="md" width={180}>
          ...
          <Menu.Dropdown>
            <Menu.Item
              onClick={() => navigate(`/accounts/employees/it/${row.id}`)}
            >
              See Profile
            </Menu.Item>
...
export function ITEmployees() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["accounts", "employees", "it", searchQuery, perPage],
  ...
  return (
    ...
    >
      <AppTable
        ...
        onRowClick={(row) => navigate(`/accounts/${tab}/it/${row.id}`)}
      />
    </PageCard>
  );
}
