      ...
      const toggleLead = useMutation({
        ...
        onSuccess: () => {
          queryClient.invalidateQueries({
            queryKey: ["accounts", "employees", "human-resources"],
          });
        },
      });
      ...
      return (
        <Menu shadow="md" width={180}>
          ...
          <Menu.Dropdown>
            <Menu.Item
              onClick={() =>
                navigate(`/accounts/employees/human-resources/${row.id}`)
              }
            >
              See Profile
            </Menu.Item>
...
export function HumanResourcesEmployees() {
  const navigate = useNavigate();
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["accounts", "employees", "human-resources", searchQuery, perPage],
  ...
  return (
    ...
    >
      <AppTable
        ...
        onRowClick={(row) =>
          navigate(`/accounts/${tab}/human-resources/${row.id}`)
        }
      />
    </PageCard>
  );
