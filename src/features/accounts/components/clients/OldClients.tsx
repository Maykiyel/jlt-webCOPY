  ...
  },
  {
    ...
    render: (row) => {
      ...
      const deactivate = useMutation({
        ...
        onSuccess: () =>
          queryClient.invalidateQueries({ queryKey: ["accounts", "clients", "old"] }),
      });
      ...
      return (
        <Menu shadow="md" width={180}>
          ...
          <Menu.Dropdown>
            <Menu.Item onClick={() => navigate(`/accounts/clients/old/${row.id}`)}>
              See Profile
            </Menu.Item>
...
export function OldClients() {
  const navigate = useNavigate();
  const { category, subCategory } = useParams();
  const tab = category || "clients";
  const sub = subCategory || "old";
  const [search, setSearch] = useState("");
  const [searchQuery, setSearchQuery] = useState("");
  const [perPage, setPerPage] = useState(10);

  const { data, isLoading } = useQuery({
    queryKey: ["accounts", "clients", "old", searchQuery, perPage],
